...
export default function AdminStatusPesananIndexPage({ auth, pagination }: PageProps<{
    pagination: PaginationData<StatusPesanan[]>;
}>) {
    ...
    const handleUpdateStatus = (id: number, newStatus: boolean) => {
        ...
        axios.post(route('status-pesanan.update-status'), {
            id: id,
            status: newStatus,
        })
            .then((res) => {
    ...
    const columns: ColumnDef<StatusPesanan>[] = [
        {
            accessorKey: "nama",
            header: ({ column }) => {
                return (
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                const currStatus = Boolean(originalRow.is_cancelable);
    ...
    return (
        <AdminLayout auth={auth}>
            <Head title="Admin - Manajemen Status Pesanan" />
            <CardTitle>
                Manajemen Status Pesanan
            </CardTitle>
            <CardDescription>
                Data Status Pesanan yang terdaftar
            </CardDescription>
                    ...
                    <AlertDialogContent className="my-alert-dialog-content" onOpenAutoFocus={ (e) => e.preventDefault() }>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription className="text-foreground">
                                Menambahkan Status Pesanan baru, contoh: <strong>Alat Diagnostik</strong>
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <form className={ cn("grid items-start gap-4") } onSubmit={ handleCreateFormSubmit }>
                            <div className="grid gap-2">
                                <Label htmlFor="nama">Nama Status Pesanan</Label>
                                <Input
                                    type="text"
                                    name="nama"
                                    id="nama"
            ...
            <AlertDialog open={ openUpdateForm } onOpenChange={ handleOpenUpdateFormChange }>
                <AlertDialogContent className="my-alert-dialog-content" onOpenAutoFocus={ (e) => e.preventDefault() }>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Anda akan mengubah nama Status Pesanan
                        </AlertDialogDescription>
                    </AlertDialogHeader>
